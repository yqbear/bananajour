class Bananajour::Bonjour::Advertiser
  ...
  private
    def register_app
      ...
      DNSSD.register("#{Bananajour.config.name}'s bananajour", "_http._tcp,_bananajour", 'local', Bananajour.web_port, tr)
    end
    def register_repos
      loop do
        stop_old_services
    ...
    def register_new_repositories
      new_repositories.each do |new_repo|
        ...
        service = DNSSD.register(new_repo.name, "_git._tcp,_bananajour", 'local', 9418, tr)
